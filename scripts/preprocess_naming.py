...
def set_name_column(source: Path, target: Path):
    ...
    con.execute("UPDATE data_table SET name = carrier || '-' || tech WHERE name IS NULL")
    con.execute(
...
set_name_column(RAW_DIR / "ts_load_electricity.csv", PREPROCESSED_DIR / "ts_load_electricity.csv")